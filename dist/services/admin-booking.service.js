...
function getManagementStatusLabel(bookingStatus, paymentStatus) {
    ...
    if (bookingStatus === "pending" && paymentStatus === "failed")
...
async function createAuditLog(tx, params) {
    await tx.auditLog.create({
        data: {
            ...
            oldData: params.oldData === undefined ? Prisma.JsonNull : normalizeJson(params.oldData),
...
function getActorLocationId(actor) {
    if (isOwner(actor)) {
        return null;
    }
    if (!actor.locationId) {
...
function ensureActorCanAccessBooking(actor, bookingLocationId) {
    if (isOwner(actor)) {
        return;
    }
...
function ensureActorCanAccessLocation(actor, locationId) {
    if (isOwner(actor)) {
        return;
    }
    const actorLocationId = getActorLocationId(actor);
...
async function resolveAddOns(tx, addOnsPayload) {
...
async function loadBookingOrThrow(tx, bookingId) {
...
async function refreshBookingForAdmin(tx, bookingId) {
    return tx.booking.findUniqueOrThrow({
        where: { id: bookingId },
        include: bookingAdminInclude,
    });
}
function buildBookingFilters(params) {
    const where = {};
    if (params.status) {
...
export const AdminBookingServices = {
    async getBookings(actor, params) {
    ...
    async getBookingDetail(actor, bookingId) {
    ...
    async getBookingLogs(actor, bookingId) {
        const booking = await prisma.booking.findUnique({
            where: { id: bookingId },
            select: {
                id: true,
    ...
    async updateBooking(actor, bookingId, payload) {
        const result = await prisma.$transaction(async (tx) => {
            ...
            const nextCustomerName = payload.customerName === undefined ? booking.customerName : String(payload.customerName).trim();
            const nextCustomerPhone = payload.customerPhone === undefined ? booking.customerPhone : String(payload.customerPhone).trim();
            const nextParticipantCount = payload.participantCount === undefined ? booking.participantCount : Number(payload.participantCount);
            const nextPackageId = payload.packageId === undefined ? booking.packageId : String(payload.packageId).trim();
            if (!nextCustomerName || nextCustomerName.length < 2)
                throw new Error("Nama customer minimal 2 karakter");
            if (!nextCustomerPhone || nextCustomerPhone.length < 8)
            ...
            if (nextParticipantCount > nextPackage.maxCapacity ||
    ...
    async cancelBooking(actor, bookingId, payload) {
        ...
        if (result.hadPendingPayment) {
            await NotificationServices.notifyPaymentFailed(result.booking.bookingId, "Pembayaran dinyatakan gagal karena booking dibatalkan oleh admin.");
        }
        return {
    ...
    async updateBookingStatus(actor, bookingId, payload) {
