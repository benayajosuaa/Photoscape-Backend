...
function parseRange(query, fallback = 'monthly') {
    const now = new Date();
...
function buildPeriodLabel(date, period) {
    if (period === 'monthly') {
...
async function getUserStatusMap(userIds) {
    if (userIds.length === 0)
        return new Map();
    ...
    for (const log of logs) {
        if (statusMap.has(log.entityId))
            continue;
        const active = log.newData?.isActive;
...
export const OwnerAdminServices = {
    ...
    async resetUserPassword(actor, userId, payload) {
    ...
    async setUserStatus(actor, userId, payload) {
    ...
    async getCustomers(_actor, query) {
        const { page, limit, skip } = parsePagination(query);
        const search = query.search?.trim();
        const where = {
            role: 'customer',
        ...
        for (const log of blacklistLogs) {
            if (blacklistMap.has(log.entityId))
                continue;
            const blacklisted = log.newData?.blacklisted;
    ...
    async getCustomerDetail(_actor, customerId) {
    ...
    async updateCustomer(actor, customerId, payload) {
    ...
    async setCustomerBlacklist(actor, customerId, payload) {
        const user = await prisma.user.findUnique({ where: { id: customerId } });
        if (!user || user.role !== 'customer')
            throw new Error('Customer tidak ditemukan');
    ...
    async getTransactions(_actor, query) {
        const { page, limit, skip } = parsePagination(query);
        const { start, end } = parseRange(query, 'monthly');
        ...
        const filtered = search
            ? rows.filter((item) => {
                ...
                return (item.booking.bookingCode.toLowerCase().includes(q) ||
    ...
    async confirmTransaction(actor, transactionId) {
    ...
    async refundTransaction(actor, transactionId, payload) {
        const tx = await prisma.payment.findUnique({ where: { id: transactionId } });
        if (!tx)
            throw new Error('Transaksi tidak ditemukan');
        const updated = await prisma.payment.update({
    ...
    async exportTransactions(_actor, query) {
        const { start, end } = parseRange(query, 'monthly');
    ...
    async getReportsDashboard(actor, query) {
        return this.getDashboard(actor, query);
    },
    async getReportsRevenue(_actor, query) {
        const { start, end } = parseRange(query, 'monthly');
    ...
    async getReportsBookings(_actor, query) {
        const { start, end } = parseRange(query, 'monthly');
        const period = normalizePeriod(query.period || 'daily');
    ...
    async getReportsStudioUsage(_actor, query) {
        const { start, end } = parseRange(query, 'monthly');
    ...
    async getSchedules(_actor, query) {
        ...
        for (const log of blocks) {
            ...
            const payload = log.newData;
            if (!payload?.studioId || !payload?.startTime || !payload?.endTime)
                continue;
    ...
    async blockSchedule(actor, payload) {
        ...
        if (!studioId || !startTime || !endTime)
            throw new Error('studioId, startTime, endTime wajib diisi');
    ...
    async unblockSchedule(actor, blockId) {
        await AuditLogServices.write({
    ...
    async updateOperationalHours(actor, payload) {
...
//# sourceMappingURL=owner-admin.service.js.map