...
export const PAYMENT_METHODS: PaymentMethod[] = ["qris", "bca_va", "mandiri_va", "gopay", "ovo", "cash"];
const SIMULATED_AUTO_SUCCESS_METHODS: PaymentMethod[] = ["bca_va", "mandiri_va", "gopay", "ovo"];
...
export function isVirtualAccountMethod(value: PaymentMethod) {
  return value === "bca_va" || value === "mandiri_va";
}

export function isSimulatedAutoSuccessMethod(value: PaymentMethod) {
...
export async function expireStaleBookings() {
  const now = new Date();
  const expiredPendingPaymentBookingIds: string[] = [];
  ...
  await prisma.$transaction(async tx => {
    ...
    expiredPendingPaymentBookingIds.push(
      ...staleBookings.filter(item => item.payment?.status === "pending").map(item => item.id)
    );
  ...
  for (const bookingId of expiredPendingPaymentBookingIds) {
    await NotificationServices.notifyPaymentFailed(
      bookingId,
      "Pembayaran tidak diselesaikan sampai melewati batas waktu."
    );
  }
}

export async function finalizePaidBooking(tx: Prisma.TransactionClient, paymentId: string, paidAt: Date) {
  ...
  const ticketExpiry = addMinutes(
    payment.booking.schedule.startTime,
    getEffectivePackageDurationMinutes({
...
export function scheduleSimulatedPaymentSuccess(paymentId: string) {
  setTimeout(async () => {
    try {
...
export function getPaymentInstructions(method: PaymentMethod) {
  const shared = [
...
export function buildQrisPaymentPageUrl(paymentId: string) {
  return `${getBaseUrl()}/api/bookings/payments/${paymentId}/qris`;
}

export async function getQrisPaymentPage(paymentId: string) {
  ...
  if (payment.method !== "qris") throw new Error("Halaman ini hanya untuk pembayaran QRIS");
  if (payment.status === "paid") {
...
export async function confirmQrisPaymentFromPage(paymentId: string) {
  await syncBookingPayments();
  ...
  const payment = await prisma.$transaction(async tx => {
    ...
    if (!currentPayment) throw new Error("Payment tidak ditemukan");
    if (currentPayment.method !== "qris") throw new Error("Halaman ini hanya untuk pembayaran QRIS");
