...
type BookingNotificationContext = Awaited<ReturnType<typeof loadBookingNotificationContext>>;
...
async function createNotifications(payloads: NotificationPayload[]) {
...
async function sendCustomerEmail(user: Pick<User, "email" | "name"> | null, params: {
  html: string;
  subject: string;
  text: string;
}) {
...
function buildBookingMetadata(booking: BookingNotificationContext, extra?: Record<string, unknown>): Prisma.InputJsonValue {
  return {
...
async function createCustomerNotification(booking: BookingNotificationContext, params: {
  emailHtml: string;
  emailSubject: string;
  emailText: string;
  message: string;
...
async function hasReminderBeenSent(bookingId: string, userId: string) {
...
export const NotificationServices = {
  ...
  async notifyPaymentPending(bookingId: string) {
    const booking = await loadBookingNotificationContext(bookingId);
  ...
  async notifyPaymentPaid(bookingId: string) {
    const booking = await loadBookingNotificationContext(bookingId);
    ...
    const addOnLines = booking.addOns.length
      ? booking.addOns.map(
          item => `${item.addOn.name} x${item.quantity} (${formatMoney(item.quantity * item.addOn.price)})`
        )
      : ["Tidak ada add-on."];
  ...
  async notifyPaymentFailed(bookingId: string, reason?: string) {
    const booking = await loadBookingNotificationContext(bookingId);
  ...
  async notifyBookingReminder(bookingId: string) {
    const booking = await loadBookingNotificationContext(bookingId);
    ...
    const run = async () => {
      ...
      try {
        ...
        if (isDatabaseConnectionError(error)) {
          ...
          console.error(
            "notification reminder job suspended: database is unreachable. restart server after DB connection is healthy.",
            error
          );
          return;
        }
