...
async function runAdminInteractiveTransaction<T>(runner: (tx: Prisma.TransactionClient) => Promise<T>) {
...
function ensureAdminCanReschedule(status: BookingStatus) {
...
export const AdminBookingServices = {
  ...
  async rescheduleBooking(actor: AdminActor, bookingId: string, payload: RescheduleBookingPayload) {
    ...
    const result = await runAdminInteractiveTransaction(async tx => {
      ...
      if (nextSchedule.startTime < getNowScheduleClock()) throw new Error("Jadwal tujuan sudah lewat");
  ...
  async updateBookingStatus(actor: AdminActor, bookingId: string, payload: UpdateBookingStatusPayload) {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entityId: booking.id,
        bookingId: booking.id,
        oldData: normalizeJson(beforeSnapshot) as Prisma.InputJsonValue,
